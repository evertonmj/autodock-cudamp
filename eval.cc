...
#ifdef CUDA_READY
  #ifdef DEBUG
   // fprintf(stderr, "CUDA(eval.cc):(operator()(Representation)\tEval class operator\n");
  #endif
#endif
...
#ifdef CUDA_READY
   #ifdef DEBUG
    //fprintf(stderr, "CUDA(eval.cc):(operator()(Representation, int term)\tEval class operator\n");
   #endif
#endif
   make_state_from_rep(rep, &stateNow);
   return eval(term);
...
#ifdef CUDA_READY
   #ifdef DEBUG
   // fprintf(stderr, "CUDA(eval.cc):(eval()\tEnergy calc\n");
   #endif
#endif
   register int i;
   int   B_outside = 0;
...
#ifdef CUDA_READY
   #ifdef DEBUG
    //fprintf(stderr, "CUDA(eval.cc):(eval(int term)\tEnergy calc term = %d\n", term);
   #endif
#endif
   register int i;
   int   B_outside = 0;
