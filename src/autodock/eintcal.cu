/*
 ...
 $Id: eintcal.cu,v 1.34 2015/11/11 21:04 mp Exp $
...
Copyright (C) 2009 The Scripps Research Institute. All rights reserved.
 ...
 File adapted from trilinterp.cc to CUDA
...
//Calculate internal energy with cuda
...
__global__ calculateEnergyParallel(
        int nb_group_max,
    ...
    //index for parallel generation
    int nb_group = blockIdx.x * blockIdx.x + threadIdx.x;

    if (nb_group < nb_group_max) {
...
#define U2 "  ________ ________  ________   ________   ____ __________"
        ...
        for (int inb = inb_from; inb < inb_to; inb++) {
            ...
            r2 = sqhypotenuse(dx, dy,
                              dz); // r2, the square of the separation between the atoms a1 and a2 in this non-bond, inb,
            ...
            if (r2 < nbc2) {
                ...
                int index_lt_NEINT = BoundedNeint(
                        index);  // guarantees that index_lt_NEINT is never greater than (NEINT - 1) (scaled NBC, non-bond cutoff)
        ...
        if (group_energy != NULL)
            switch (nb_group) {
                ...
                case INTRA_RECEPTOR:  // [2]  intramolecular energy of receptor
                    ...
                    break;
                default:
                    stop("bug: bad group index in eintcal");
            }
    }
}
 ...
 *  11/11/15 EMJ     Translated into CUDA C                                    */
/* *****************************************************************************/

{
    ...
    //host memory
    int *host_memory;
    int dimA = nb_group_max;
    //device memory
    int *device_memory;

    //kernel blocks and threads
    int numBlocks = 16;
    int numThreadsPerBlock = 64;

    //memory total for task
    size_t memSize = numBlocks * numThreadsPerBlock * sizeof(nb_group_max);

    //memoy allocation and copy
    host_memory = (int *) malloc(memSize);
    cudaMalloc((void **) &device_memory, memSize);
    cudaMemcpy(device_memory, host_memory, memSize, cudaMemcpyHostToDevice);
    ...
    //call kernel
    calculateEnergyParallel<<< dimGrid, dimBlock >>> (
            nb_group_max,
                    nonbondlist,
                    ptr_ad_energy_tables,
                    tcoord[MAX_ATOMS][SPACE],
                    Nnb,
                    Nnb_array,
                    group_energy,
                    B_calcIntElec,
                    ...
                    scale_1_4,
                    qsp_abs_charge[MAX_ATOMS],
                    B_use_non_bond_cutoff,
