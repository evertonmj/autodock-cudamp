...
class Phenotype
{
   ...
   public:
      ...
      void writeQuat( Quat q );
#ifdef CUDA_READY
      unsigned int getEvalFlag();
      void setEvalFlag(unsigned int flag);
...
#ifdef CUDA_READY
inline void Phenotype::setValue(double val)
{
    value = val;
}

inline unsigned int Phenotype::getEvalFlag(void)
{
    return evalflag & 1;
}

inline void Phenotype::setEvalFlag(unsigned int flag)
{
    evalflag = flag;
}

inline Representation ** Phenotype::getRep(void)
{
    return value_vector;
}
inline double Phenotype::getValue(void)
{
    return value;
}
#endif
...
{
#ifdef CUDA_READY
//    fprintf(stderr, "CUDA(support.h):value(EvalMode)\tcalling evaluate\n");
#endif
